...
const Clienty = React.memo(({ navigation, route }) => {
  ...
  const sendDataClient = async (invoiceDataClient) => {
   ...
   const buttonStyle = {
        ...
        // Function to handle scroll event and show/hide the arrow
       const handleScroll = (event) => {
         const { contentOffset, contentSize, layoutMeasurement } = event.nativeEvent;
         const isScrolledToEnd = contentOffset.y + layoutMeasurement.height >= contentSize.height - 10; // 10-pixel threshold

         setShowArrow(!isScrolledToEnd);
       };

       // Set initial arrow visibility based on content size when content is loaded
       const handleContentSizeChange = (contentWidth, contentHeight) => {
         setShowArrow(contentHeight > screenHeight); // Show arrow if content exceeds screen height
       };
