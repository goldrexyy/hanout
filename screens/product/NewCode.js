...
const NewCode = ({ AddCode, details }) => {
  const [product, setProduct] = useState({
    ...
    timesSold: 0,
    avatar: '',
    ean: '', // Use item.codes as EAN
    exists: false, // Mark as a known product
  });
 ...
 suggestionText: {
  ...
  const handleInputChangeEAN = (text) => {
    // Set the product.ean to the concatenation of the previous value and the new text
    setProduct((prevProduct) => ({
      ...prevProduct,
      ean:text, // Append the new character to the existing value
    }));
  };
  ...
  return (
    <Layout style={styles.container}>
      ...
      <TouchableOpacity >
        <Text style={styles.Selecttext14}>Code produit</Text>
        <Text style={styles.Selecttext144}>Ajoutez un produit personnalisé en lui attribuant un code unique selon sa catégorie. Une fois le code défini, ce produit pourra être ajouté directement à la facture en le saisissant au clavier, sans nécessiter de recherche manuelle. Cette méthode accélère l'ajout de produits spécifiques en permettant une entrée rapide et efficace, en simplifiant le processus de facturation.</Text>
      </TouchableOpacity>
      ...
      {/* BottomSheetTextInput with initial value from product.name */}
           <BottomSheetTextInput
             placeholder="Entez le nom de votre produit..."
   ...
   {suggestions.map((suggestion, index) => (
     ...
     >
       <Text style={styles.suggestionText}>{suggestion}</Text>
        ...
        <TouchableOpacity >
          <Text style={styles.Selecttext14}>Prix de vente en Dh </Text>
...
onPressOut={handlePressOut}
          ...
          <Icons.Ionicons name="add-circle-sharp" size={width * 0.2} color="#333" />
              ...
              <TouchableOpacity >
                <Text style={styles.Selecttext14}>Code attribué : {details.prefixcategory}{product.ean} </Text>
                <Text style={styles.Selecttext144}>Ajoutez un complément au code de famille {details.prefixcategory} pour finaliser votre code produit unique. Une fois le complément saisi, votre code produit complet sera {details.prefixcategory} + {product.ean}, facilitant ainsi l’identification et la gestion de ce produit spécifique.</Text>

              </TouchableOpacity>

              <BottomSheetTextInput
                placeholder="Entrez un code à ce produit.. Exemple : 1"
                value={product.ean} // Use product.ean as value
                onChangeText={handleInputChangeEAN} // Update the input handler
                style={styles.input}
        ...
        <View style={styles.ButtonCreatInvoiceContainer} >
          <Button
            ...
            onPress={()=>AddCode(product)}
            disabled={!product.name}
          >
            {details ? 'AJOUTER CODE' : 'AJOUTER CODE'}
          </Button>
        </View>
