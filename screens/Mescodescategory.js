...
const Mescodescategory = ({ navigation, route  }) => {
  ...
  const filteredCategories = categories.filter((category) =>
    ...
    category.idUnique.slice(0, 3) === details.prefix
  );
  ...
  const handlePresentModalFirstPress = useCallback((component) => {
    // If it's a string, handle it as a component type
     ...
     if (component === 'AJOUTER') {
        setSnapPoint(['80%']);  // Set snap point for 'Modify'
      } else {
        setSnapPoint(['80%']);  // Default snap point for other cases
      }
  ...
  const handlePress = (item) => {
               ...
               const prefixcategory = item.idUnique.substring(0, 4);


               // Navigate to the Mescodescategory screen and send data
               navigation.navigate('Mescodesproducts', {
                 details: {
                   name: details.name,
                   prefix: details.prefix,
                   namecategory : item.nom,
    ...
    <ApplicationProvider {...eva} theme={eva.light}>
      ...
      <View style={styles.formContainerSearch}>
            <TextInput
              ...
              placeholder="Trouver une sous catégorie catégorie.."
              value={searchTerm}
...
export default Mescodescategory;
